...
import ResultSearchBarTemplate from "./templates/result-search-bar";
import ResultSearchBarResultsTemplate from "./templates/result-search-bar-results";
...
if (document.getElementById('result-search-bar')) {
    const resultSearchBar = (new AutocompleteBar({
        el: `#result-search-bar`, // The element the AutocompleteBar will be put in.
        search: {
        ...
        query: `q`, // How are we naming the search query in the URL.
        ...
        fill_input_on_click: false, // When we click a record in the results it will fill the input field with the data of that record.
        ...
        show_results: false, // It's not always necessary to show the dropdown with results. Setting this option to false will disable showing the dropdown.
    ...
    function submitResultSearchBar(search = null) {
        ...
        let c = url.searchParams.get(resultSearchBar.getQuery());
    ...
    document.addEventListener((Events.onSubmitSearch), (_) => {
        ...
        if (_.detail.value.url !== undefined && _.detail.value.url !== null) {
            ...
            submitResultSearchBar(_.detail.value);
        }
    });
